...
- `shops` calls Google Places Nearby Search **live on every request** (ratings are current),
  computes distance from the user's coords (haversine), and adds an installed-price
  *estimate* (Claude if a key is set, otherwise a heuristic table).
